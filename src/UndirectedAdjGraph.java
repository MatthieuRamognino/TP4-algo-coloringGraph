...
//		Initialisation de la liste colors
		for (int i=0; i<this.vertices().size(); i++) {
			// 0 correspond � un sommet sans couleur
			colors.add(0);
		}
		
		// parcours le graph et cr�e une liste o� chaque valeur correspond au degr�e d'un sommet
		...
		// tri cette liste dans l'ordre d�croissant
		...
		while(this.colors.contains(0)) {
			...
			for (int i=0;i<degreesTrie.size();i++) {
				...
				// prend un sommet regarde si ce sommet nest pas d�ja color�
				// de plus regarde si il ne poss�de pas dans ses voisins un sommets color�e
				for (int k=0; k<degreesTrie.size(); k++) {
					List<Vertex> voisinsDuSommets = this.outNeighbors(this.vertices().get(degreesTrie.get(k)));
					// si le sommet nes pas color�
					if (this.colors.get(degreesTrie.get(k)) == 0) {
						...
						for (int j=0;j<voisinsDuSommets.size();j++) {
							// on regarde si ils sont color� ou non
							if (this.colors.get((int) voisinsDuSommets.get(j)) == colors.get(degreesTrie.get(i))) { // il faut ajouter d'autre condition
								// System.out.println("poss�de un voisin color�");
								voisinColoree = true;
							}	
						}
						// si il ne poss�de aucun voisin coloree alors on le colore dans la m�me couleur que le sommet degreesTrie
